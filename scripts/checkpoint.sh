...
if [ "${1:-}" = "--prune" ]; then
    ...
    done < <(git tag -l "quantdev-checkpoint-*" 2>/dev/null)
...
TAG="quantdev-checkpoint-${LABEL}-${TIMESTAMP}"

if ! git rev-parse --git-dir >/dev/null 2>&1; then
    echo "Warning: Not in a git repository; skipping checkpoint tag" >&2
    exit 0
fi
git tag -a "$TAG" -m "Quantdev checkpoint: ${LABEL}" 2>/dev/null || {
