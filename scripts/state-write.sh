#!/usr/bin/env bash
# Quantdev state writer
# Updates .quantdev/STATE.json with current position and status
# Appends to .quantdev/HISTORY.md
#
# Usage:
...
#   3 - Missing dependency (.quantdev/ directory missing, mktemp failed)
...
if [ -L ".quantdev" ]; then
    echo "Error: .quantdev is a symlink, which is not allowed" >&2
    exit 3
fi

# Ensure .quantdev directory exists
if [ ! -d ".quantdev" ]; then
    echo "Error: .quantdev/ directory does not exist. Run /quantdev:init first." >&2
    exit 3
fi

STATE_FILE=".quantdev/STATE.json"
HISTORY_FILE=".quantdev/HISTORY.md"
...
if [ "${QUANTDEV_TEAMS_ENABLED:-}" = "true" ]; then
    ...
    MAX_RETRIES="${QUANTDEV_LOCK_MAX_RETRIES:-60}"
    RETRY_DELAY="${QUANTDEV_LOCK_RETRY_DELAY:-0.1}"
...
if [ -n "$STATUS" ]; then
    case "$STATUS" in
        ready|planned|planning|building|in_progress|complete|complete_with_gaps|shipped|blocked|paused|backtesting|optimizing|researching)
            ;;
        *)
            echo "Error: --status must be one of: ready, planned, planning, building, in_progress, complete, complete_with_gaps, shipped, blocked, paused, backtesting, optimizing, researching. Got '${STATUS}'" >&2
            exit 1
            ;;
    esac
fi
...
if [ -n "$TRACK" ]; then
    case "$TRACK" in
        indicator|bot|research|"")
            ;;
        *)
            echo "Error: --track must be one of: indicator, bot, research. Got '${TRACK}'" >&2
            exit 1
            ;;
    esac
fi

# Recovery mode: rebuild STATE.json from .quantdev/ artifacts
if [ "$RECOVER" = true ]; then
    echo "Recovering STATE.json from .quantdev/ artifacts..." >&2

    # Find latest phase number from phases/ directories
    latest_phase=""
    if [ -d ".quantdev/phases" ]; then
        latest_phase=$(find .quantdev/phases/ -maxdepth 1 -type d 2>/dev/null | \
    ...
    phase_dir=".quantdev/phases/${latest_phase}"
    if [ -d "$phase_dir" ]; then
    ...
    if command -v git >/dev/null 2>&1 && git rev-parse --git-dir >/dev/null 2>&1; then
        while IFS= read -r tag; do
            ...
            tag_label=$(printf '%s\n' "$tag" | sed 's/^quantdev-checkpoint-//' | sed 's/-[0-9]*T[0-9]*Z$//')
            recovered_history="${recovered_history}- [${tag_date:-unknown}] Checkpoint: ${tag_label}
"
        done < <(git tag -l "quantdev-checkpoint-*" 2>/dev/null | sort)
    fi
    ...
    # Write history
    append_history "State recovered from .quantdev/ artifacts"
...
if [ -n "$PHASE" ] || [ -n "$POSITION" ] || [ -n "$STATUS" ]; then
    NEW_CONTENT=$(jq -n \
        ...
        '{schema: $schema, phase: $phase, position: $position, status: $status, updated_at: $updated_at, blocker: (if $blocker == "" then null else $blocker end), track: (if $track == "" then null else $track end), strategy: (if $strategy == "" then null else $strategy end)}')
    cp "$STATE_FILE" "${STATE_FILE}.bak" 2>/dev/null || true
    atomic_write "$NEW_CONTENT" "$STATE_FILE"
    shasum -a 256 "$STATE_FILE" | cut -d' ' -f1 > "${STATE_FILE}.sha256"
    history_entry="Phase ${PHASE:-?}: ${POSITION:-updated} (${STATUS:-unknown})"
    [ -n "$TRACK" ] && history_entry="${history_entry} [${TRACK}]"
    [ -n "$STRATEGY" ] && history_entry="${history_entry} strategy=${STRATEGY}"
...
if [ -n "$NOTE_TEXT" ]; then
    printf '%s\n' "- [${TIMESTAMP}] ${NOTE_TEXT}" >> ".quantdev/NOTES.md"
