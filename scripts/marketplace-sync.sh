#!/usr/bin/env bash
#
# Keeps the Quantdev marketplace clone in sync with GitHub.
...
QUANTDEV_CONFIG_DIR="${HOME}/.config/quantdev"
SYNC_LOG="${QUANTDEV_CONFIG_DIR}/marketplace-sync.log"
SYNC_TIMESTAMP="${QUANTDEV_CONFIG_DIR}/marketplace-sync.last"
SYNC_COOLDOWN=3600  # 1 hour

MARKETPLACE_DIR="${HOME}/.claude/plugins/marketplaces/quantdev"

mkdir -p "${QUANTDEV_CONFIG_DIR}"
