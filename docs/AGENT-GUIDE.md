...
```mermaid
graph LR
    subgraph init ["/quantdev:init"]
        S[settings collection]
    end
    subgraph brainstorm ["/quantdev:brainstorm"]
        BR[brainstorming] --> A0[architect]
    end
    subgraph plan ["/quantdev:plan"]
        R[researcher] --> A2[architect]
        A2 --> V1[verifier]
    end
    subgraph build ["/quantdev:build"]
...
- **Dispatched by:** `/quantdev:brainstorm` (roadmap), `/quantdev:plan` (roadmap + plans), `/quantdev:quick` (quick plans)
...
- **Dispatched by:** `/quantdev:build` (per-plan), `/quantdev:quick`
...
- **Dispatched by:** `/quantdev:build` (after each plan completes), `/quantdev:review` (on-demand)
- **Recommended max_turns:** 15
...
- **Restrictions:**
  ...
  - Non-blocking findings appended to `.quantdev/ISSUES.md`
...
- **Dispatched by:** `/quantdev:plan` (plan quality), `/quantdev:build` (phase completion), `/quantdev:ship` (final validation), `/quantdev:verify` (on-demand)
- **Recommended max_turns:** 15
...
- **Dispatched by:** `/quantdev:build` (after verification), `/quantdev:ship` (mandatory, ignores config), `/quantdev:audit` (on-demand)
- **Recommended max_turns:** 15
- **Inputs:** Git diff of all phase changes, PROJECT.md, CONVENTIONS.md, dependency manifests
- **Outputs:** AUDIT-{N}.md
- **Restrictions:**
  - Critical findings block shipping — phase must not proceed to `/quantdev:ship`
...
- **Dispatched by:** `/quantdev:build` (after auditor, unless `--light`), `/quantdev:simplify` (on-demand)
...
- **Restrictions:**
  ...
  - Deferred findings appended to `.quantdev/ISSUES.md`
...
- **Dispatched by:** `/quantdev:build` (after simplifier, unless `--light`), `/quantdev:ship` (comprehensive), `/quantdev:document` (on-demand)
...
- **Dispatched by:** `/quantdev:plan` (before architect, unless `--skip-research`), `/quantdev:research` (on-demand)
- **Recommended max_turns:** 15
...
- **Dispatched by:** `/quantdev:debug` (on-demand), `/quantdev:build` (optionally, on persistent critical issues)
- **Recommended max_turns:** 20
...
- **Dispatched by:** `/quantdev:map` (on-demand, supports all 4 focus areas in parallel)
- **Recommended max_turns:** 20
...
- **Parallel dispatch:** `/quantdev:map` runs 4 mapper instances concurrently, each with a different focus area.
...
- **auditor** — Critical findings block `/quantdev:ship`
...
```
Standard `/quantdev:build` flow. The `[builder → reviewer]*` loop repeats up to 2 retries on critical findings. Auditor, simplifier, and documenter run after phase verification.

### Quick Task
```
architect → builder
```
Simplified `/quantdev:quick` flow. Architect produces a lightweight plan (max 3 steps), builder executes it. No review gate.
...
```
Standard `/quantdev:plan` flow. Researcher investigates the domain, architect decomposes into plans, verifier checks plan quality and coverage.
