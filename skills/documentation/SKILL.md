...
### Strategy Journal Maintenance

Every strategy gets a living `JOURNAL.md` in `.quantdev/strategies/{name}/`:

```markdown
# Keltner Breakout Strategy Journal

## 2026-02-14 — Initial Design
- Hypothesis: Keltner channel breakouts with ADX filter capture trending moves
- Target regime: Trending (ADX > 25)
- Parameters: period=20, atr_mult=1.5, adx_threshold=25

## 2026-02-15 — First Backtest
- ES 2020-2024: PF 2.4, Sharpe 1.8, 186 trades (in-sample)
- Observation: bleeds in ranging markets (PF 0.7 when ADX < 20)
- Decision: Add ADX gate, only enter when ADX > 25

## 2026-02-16 — Parameter Sensitivity
- ATR mult 1.0-3.0: plateau around 1.5-2.0, cliff below 1.0
- ADX threshold 20-30: stable performance, 25 is sweet spot
```

Update the journal with EVERY significant change, observation, or decision.

### Knowledge Base Entries

Key discoveries go in `.quantdev/KNOWLEDGE.md`:

```markdown
## EMA Calculation Differences
Pine Script's `ta.ema()` uses standard EMA (2/(N+1) multiplier).
Pine Script's `ta.rma()` uses Wilder's smoothing (1/N multiplier).
NinjaScript's `EMA()` matches Pine's `ta.ema()`.
When porting, always verify which variant is used.
```

### Parameter Documentation

Every strategy parameter needs rationale:

```json
{
  "atr_period": {
    "value": 14,
    "rationale": "Standard ATR period, matches TradingView default",
    "sensitivity": "Stable across 10-20 range, cliff at 5"
  }
}
```

### Indicator API Docs Per Platform

For each platform implementation, document:
- Function signature with parameter types
- Return value(s) and their meaning
- Warm-up period requirement
- Known differences from other platform versions

## Integration

**Referenced by:** `quantdev:documenter` agent
**Pairs with:** `quantdev:quantdev-verification` (documentation completeness is part of "done"), `quantdev:lessons-learned` (findings feed knowledge base)
