...
### Trading Evidence Requirements

Before claiming ANY trading work is complete, verify per track:

**Indicator Track:**
- [ ] Golden-file test executed — output matches reference within epsilon
- [ ] Cross-platform validation passed (`/quantdev:validate`) — or N/A if single platform
- [ ] Lookahead check clean — no future data access (`quantdev:lookahead-guard` checklist passed)
- [ ] Warm-up period handled — indicator returns NaN/zero before sufficient bars

**Bot Track:**
- [ ] Unit tests pass — all `_test.go` / `test_*.py` green
- [ ] Lookahead check clean — no close-price peeking during bar formation
- [ ] Risk parameters validated — stop loss, daily limit, position size bounded (`quantdev:risk-discipline`)
- [ ] Regression baselines intact — PF, Sharpe, drawdown within tolerance of baseline

**Research Track:**
- [ ] Findings documented — FINDINGS.md or JOURNAL.md updated
- [ ] Backtest integrity checked — sample size, OOS, regime segmentation (`quantdev:backtest-integrity`)
- [ ] Statistical claims supported — confidence intervals, p-values where applicable
- [ ] Knowledge base updated — key insights added to KNOWLEDGE.md

### Teammate Mode

When `QUANTDEV_IS_TEAMMATE=true`:
