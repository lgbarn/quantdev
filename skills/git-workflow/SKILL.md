...
2. ~/.config/quantdev/worktrees/<project-name>/ (global location)
...
**For Global Directory (~/.config/quantdev/worktrees):**
...
case $LOCATION in
  .worktrees|worktrees)
    ...
    ;;
  ~/.config/quantdev/worktrees/*)
    path="~/.config/quantdev/worktrees/$project/$BRANCH_NAME"
    ;;
esac
...
- Commit messages should be descriptive and trading-contextual
- Use conventional commit prefixes: `feat:`, `fix:`, `test:`, `refactor:`, `docs:`
- Stage specific files, not `git add -A`
- Never commit secrets, credentials, API keys, or large data files

### Trading Commit Convention Examples

```
feat(keltner): add ATR period sensitivity filter
fix(vwap): correct overnight session boundary reset
test(supertrend): add regression baseline for ES 2020-2024
refactor(ema): extract common warm-up period guard
feat(bot/keltner): add daily loss limit circuit breaker
fix(backtest): handle CME maintenance window data gaps
docs(alligator): document jaw/teeth/lips period rationale
test(indicators): add golden-file tests for Pine Script parity
feat(optimizer): add walk-forward validation windows
fix(risk): bound position size to Apex account tier limits
```

### Scope Convention for Trading

| Scope | When |
|-------|------|
| `(indicator-name)` | Indicator-specific changes: `feat(keltner): ...` |
| `(bot/strategy-name)` | Bot-specific changes: `fix(bot/supertrend): ...` |
| `(backtest)` | Backtest engine changes |
| `(risk)` | Risk management changes |
| `(data)` | Data pipeline or format changes |
| `(platform)` | Platform-specific: `feat(pine): ...`, `fix(ninja): ...` |
