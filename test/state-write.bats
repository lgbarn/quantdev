...
# bats test_tags=unit
@test "state-write: fails without .quantdev directory" {
    cd "$BATS_TEST_TMPDIR"
    # Do NOT create .quantdev
    run bash "$STATE_WRITE" --phase 1 --position "test" --status ready
    assert_failure
    assert_output --partial ".quantdev"
}
...
@test "state-write: atomic write leaves no temp files" {
    ...
    run find .quantdev -name "*.tmp.*"
    assert_output ""
}

# bats test_tags=unit
@test "state-write: missing .quantdev exits code 3" {
    cd "$BATS_TEST_TMPDIR"
    run bash "$STATE_WRITE" --phase 1 --position "test" --status ready
    assert_failure
...
@test "state-write: --recover rebuilds from phase artifacts" {
    ...
    echo "# Plan" > .quantdev/phases/2/plans/PLAN-2.1.md
...
@test "state-write: creates .bak file on structured write" {
    ...
    bash "$STATE_WRITE" --phase 2 --position "Second" --status building
    [ -f .quantdev/STATE.json.bak ]

    # .bak should contain the previous state (phase 1)
    local bak_phase
    bak_phase=$(jq -r '.phase' .quantdev/STATE.json.bak)
    [ "$bak_phase" = "1" ]
}
...
@test "state-write: creates .sha256 checksum file on write" {
    setup_quantdev_dir
    bash "$STATE_WRITE" --phase 1 --position "test" --status ready
    [ -f .quantdev/STATE.json.sha256 ]

    # Checksum should match actual file
    local expected actual
    expected=$(cat .quantdev/STATE.json.sha256)
    actual=$(shasum -a 256 .quantdev/STATE.json | cut -d' ' -f1)
    [ "$expected" = "$actual" ]
}
